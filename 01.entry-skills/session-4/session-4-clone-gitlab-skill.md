...
description: "Use when: 需要根据一个或多个 GitLab 项目 ID 拉取代码、拼接仓库地址、确认项目名并执行单份 clone，并在拉取完成后调用 PromptArchitect agent 生成提示词。适用于从公司 GitLab 用 HTTPS + Access Token 拉取 prompt2repo 下的项目到 02.work session/session-4/gitlab source 目录。"
---
...
这个技能用于处理 session-4 的项目拉取入口。它负责把 GitLab 项目拉到本地、生成项目对应的 prompt，并初始化模型结果记录文件。

这个技能负责：

- 根据项目 ID 补全项目名
- 拼接 GitLab 仓库地址并执行 clone
- 调用 PromptArchitect 生成提示词
- 将结果写入 `03.trae prompts/session-4.md`
...
| 提示词文件 | D:\charles\program\ai\apps\03.trae prompts\session-4.md | prompt 落盘位置 |
...
5. 验证仓库目录存在且包含 `.git`。
6. 逐个调用 PromptArchitect 生成提示词。
7. 将提示词写入 `03.trae prompts/session-4.md`。
8. 为每个项目初始化 `ai-model-result/<项目名>.md`。
9. 向用户返回仓库地址、落盘目录和 prompt 结果。
...
在仓库拉取成功后，必须调用 PromptArchitect，并遵循 `.github/instructions/promptarchitect-session-4.instructions.md`。

执行要求：

- agent 名称必须精确使用 `PromptArchitect`
- 生成前先检查 `03.trae prompts/session-4.md` 中是否已有同项目或语义相近记录
- 若已有相近记录，先询问用户：保留旧版、替换旧版，或放弃写入
- 批量模式下，重复检查必须逐个项目执行，不能跳过

提示词写入规则：

- 文件固定写入 `03.trae prompts/session-4.md`
- 不存在时新建，存在时默认追加
- 每个项目对应一条独立记录
...
- 从 `03.trae prompts/session-4.md` 中读取该项目下的所有 prompt 条目
- 按条目数量写入对应的结果记录模板
...
2. clone 完成后不能直接结束，必须继续生成 prompt。
3. PromptArchitect 生成的提示词必须落盘到 `03.trae prompts/session-4.md`。
4. `03.trae prompts/session-4.md` 默认追加，不覆盖。
5. prompt 写入完成后，必须为每个项目检查并创建结果文件。