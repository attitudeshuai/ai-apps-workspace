...
2. 标准化项目名（如 `1035 -> label-01035`）。
3. 若主仓不存在，先执行 clone 拉取主仓。
4. 校验本轮总数不超过 16；index 使用全局累计并补零（01-16）。
5. 以项目/类型两级目录写入提示词文件，文件名格式：`A-<项目名>-<类型>-<index>.md`。
6. 每个提示词文件正文必须包含同名标识串：`A-<项目名>-<类型>-<index>`。
7. 按提示词逐条复制主仓到对应子仓目录，确保一条提示词对应一个子仓。
8. 对 Bug 修复类型，在对应 bug 子仓中设计 bug；禁止在主仓或其他子仓改动。
9. 输出汇总：成功项、失败项、文件路径、子仓路径。
...
提示词文件：A-label-01035-bug修复-01.md ... A-label-01035-代码测试-16.md
