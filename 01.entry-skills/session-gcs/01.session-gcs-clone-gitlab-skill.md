...
- 调用 `promptarchitect(.github\agents\PromptArchitect.agent.md)` agent 生成**单条**提示词
...
6. 用户确认类型后，根据类型走不同分支：
   ...
   **若类型为『Bug 修复』：**
   - 调用 `promptarchitect(.github\agents\PromptArchitect.agent.md)` agent，根据代码设计一个想实际线上会出现的 bug（逻辑、业务、状态、权限等）。
...
2. 读取 origin 仓代码，结合确认的类型**调用 `promptarchitect(.github\agents\PromptArchitect.agent.md)` agent**，传入项目名、类型（配额固定 `*1`）和 origin 仓代码目录上下文：
   - Bug 修复类型：代码中已有 bug，prompt 描述 bug 现象和触发条件，**不要把 bug 代码位置或修复方案交交。**
   - 其他类型：按类型策略正常出题。
3. 将 promptarchitect(.github\agents\PromptArchitect.agent.md) 输出的提示词**交给 `humanizer-zh` agent** 进行去 AI 化优化，确保语气自然、口语化，像真实工作里的需求或抛出的问题。
...
6. humanizer-zh 优化是必须步骤，不可跳过；promptarchitect(.github\agents\PromptArchitect.agent.md) 的原始输出不直接写入文件。
