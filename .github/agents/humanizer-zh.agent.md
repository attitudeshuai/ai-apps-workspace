...
**问题：** LLM 使用破折号（— 或中文双破折号 ——）比人类更频繁，常用于"引出阐述/转折/强调"，模仿"有力"的销售文案。中文写作中的 `——` 尤其是 AI 高频痕迹，通常可直接删掉或改成逗号、句号。

**强制执行：完成润色后，必须对输出文本做一次全文 `——` 扫描。只要发现 `——`，无论上下文如何，都必须替换掉，不允许保留。**

**替换规则（按优先级）：**
- 转折关系（"X——而是 Y" / "X——但 Y"）→ 改为逗号：`X，而是 Y` / `X，但 Y`
- 解释/补充（"原因——它从前提就错了"）→ 删掉 `——`，直接连接或加逗号
- 引出列举（"流程没问题——读文件、理解结构……"）→ 改为冒号：`流程没问题：读文件、理解结构……`
- 插入说明（"这件事——当时没人注意——后来影响很大"）→ 改为逗号或括号
- 结尾强调（"他只说了一句话——不行"）→ 改为句号或逗号分句
...
**问题：** AI 在普通叙述文字中用反引号（`）包裹方法名、字段名、命令等，制造代码格式。这是 markdown 技术文档习惯，用在自然语言写作中会产生明显的 AI 痕迹。

**强制执行：完成润色后，必须对输出文本做一次全文反引号扫描。只要发现反引号，无论上下文如何，都必须移除，不允许保留。**

**替换规则：**
- 包裹方法名/函数名 → 直接写出名称，不加任何格式符号
- 包裹字段名/变量名 → 直接写出名称，或用中文描述
- 包裹命令/路径 → 改用引号或直接嵌入句中
- 代码块（三个反引号）→ 如果是叙述文字，改写成自然语言

**改写前：**
> 调用 `getUserById` 方法时，需要先检查 `userId` 是否为空，否则会抛出 `NullPointerException`。

**改写后：**
> 调用 getUserById 时，需要先检查 userId 是否为空，否则会抛出空指针异常。
...
**注：** 简体中文的标准引号是""和''。此模式在中文中表现为英文直引号（""）的使用。

---

### 20. 「」书名号式引号

**问题：** 「」是日语和繁体中文的引号风格。AI 在简体中文写作中频繁使用这一符号，制造"有品味"的书面感，实为非本土化的 AI 痕迹。简体中文应统一使用""。

**强制执行：完成润色后，必须对输出文本做一次全文「」扫描。只要发现「」，无论上下文如何，都必须替换为""，不允许保留。**

**改写前：**
> 他提出了「效率优先」的原则，认为「结果比过程重要」。
...
**问题：** AI 按英文语法逐词映射成中文，或将多个从句强行塞入一个句子，导致读者需要反复重读才能理解，甚至读完还不知道在说什么。常见表现：
- **翻译腔**：按英文主谓宾顺序硬套，句子听起来不像中文母语者会说的话
- **长定语堆叠**：在名词前堆积过多修饰语，如"一个旨在通过整合……来解决……的全新解决方案"
- **被动语态滥用**：中文很少用被动，但 AI 频繁使用"被实现""被执行""被认为"
- **主语游移**：同一段落内主语反复切换或省略，读者不知道"谁"在做什么
- **连续多个"的"**：如"公司的战略的调整的方向"，应拆散重写

**处理原则：**
- 拆长句：把一个复合长句拆成两三个短句
- 换主动：把"X 被 Y 完成"改为"Y 完成了 X"
- 还原中文语序：把修饰成分移到动词后或独立成句
- 补明主语：确保每句话都能清楚看出谁在做什么

**改写前：**
> 通过对现有流程的深入分析以及对用户反馈的系统性整合，一套旨在提升效率并降低操作复杂度的优化方案被提出，并被计划在下季度完成部署。

**改写后：**
> 我们分析了现有流程，整理了用户反馈，提出了一套优化方案，计划下季度完成部署。目标是提升效率、降低操作难度。
...
- ✓ **连续三个句子长度相同？** 打断其中一个
- ✓ **段落以简洁的单行结尾？** 变换结尾方式
- ✓ **文中有 `——`？** 必须全部替换，一个都不能留（见规则 13）
- ✓ **文中有反引号？** 必须全部移除，不允许保留（见规则 15）
- ✓ **文中有「」？** 必须全部替换为""，不允许保留（见规则 20）
- ✓ **揭示前有破折号？** 删除它
- ✓ **解释隐喻或比喻？** 相信读者能理解
- ✓ **使用了"此外""然而"等连接词？** 考虑删除
- ✓ **三段式列举？** 改为两项或四项
- ✓ **句子读不顺、需要重读？** 拆句、换主动、补明主语（见规则 27）
